...
def updateBestDistance(node):
    ...
    if bestDistance.shape != mymap.shape:
        bestDistance = np.zeros(mymap.shape,dtype=int) + np.sum(mymap)+1
    ...
    for c in pathstr[1:]:  # Start processing at move to second point as first point is (0,0)
        if pos != [0, 0]:
            length += mymap[pos[0], pos[1]]

        if length < bestDistance[pos[0],pos[1]]:
            bestDistance[pos[0], pos[1]] = length
...
# def updateBestDistanceNode(node):
#     global bestDistance
#
#     if bestDistance.shape != mymap.shape:
...
def next_square(pos, newdir, pastdir, runlen, len, path):
    ...
    metric = mymap[temp_pos[0],temp_pos[1]] + 3*delta + 10*delta/mdistance(temp_pos)
...
def findPathGreedy(node, RecurseCnt=0):  # node = [len, [x,y], dir, runlen):
    ...
    if pos == destination:
        ...
        # updatesearched(node)
        updateBestDistance(node)

        if node['len'] < bestpath['len']:
            # toSearchQ.sort(reverse=True, key=get_dist)
    ...
    if lowestlength(node) >= bestpath['len'] + EXTRA_PATH:
        updateBestDistance(node)        # Any square searched will have its best distance updated (takes lower or existing and this value)
        return

    if node['len']  >= getBestDistance(node) + mdistance(node['pos']):
        updateBestDistance(node)
        return
...
EXTRA_PATH = 5*np.mean(mymap)       #int(mymap.size/100*np.mean(mymap))
# mypath.append(node)
findPathGreedy(node)
